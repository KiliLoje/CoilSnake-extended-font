...
from coilsnake.exceptions.common.exceptions import (
    ...
    TableEntryInvalidYmlRepresentationError,
    TableError,
    TableEntryMissingDataError,
    TableEntryError,
    TableSchemaError,
)
    ...
    def from_yml_rep(cls, yml_rep):
        if isinstance(yml_rep, bool):
            ...
            raise TableEntryInvalidYmlRepresentationError(
                "Could not parse value[{}] as a boolean. Valid values are "
...
class LittleEndianIntegerTableEntry(TableEntry):
    ...
    def create(name, size):
        return type(
            name, (LittleEndianIntegerTableEntry,), {"name": name, "size": size}
        )
...
class LittleEndianOneBasedIntegerTableEntry(LittleEndianIntegerTableEntry):
    ...
    def from_block(cls, block, offset):
        return (
            super(LittleEndianOneBasedIntegerTableEntry, cls).from_block(block, offset)
            - 1
        )

    @classmethod
    def to_block(cls, block, offset, value):
        super(LittleEndianOneBasedIntegerTableEntry, cls).to_block(
            block, offset, value + 1
        )
    ...
    def from_yml_rep(cls, yml_rep):
        if yml_rep is None:
            ...
            return super(LittleEndianOneBasedIntegerTableEntry, cls).from_yml_rep(
                yml_rep
            )
...
class EnumeratedLittleEndianIntegerTableEntry(LittleEndianIntegerTableEntry):
    ...
    def create(name, size, values):
        enumeration_class = type(
            "GenericEnum_{}".format(name),
            (GenericEnum,),
        ...
        return type(
            ...
            {"name": name, "size": size, "enumeration_class": enumeration_class},
        )
    ...
    def from_yml_rep(cls, yml_rep):
        if isinstance(yml_rep, str):
            ...
            return super(EnumeratedLittleEndianIntegerTableEntry, cls).from_yml_rep(
                yml_rep
            )
        else:
            raise TableEntryInvalidYmlRepresentationError(
                "Could not parse value[{}] as [{}]".format(yml_rep, cls.name)
            )
...
class ByteListTableEntry(TableEntry):
    ...
    def from_yml_rep(cls, yml_rep):
        if not (isinstance(yml_rep, list) and all(isinstance(x, int) for x in yml_rep)):
            raise TableEntryInvalidYmlRepresentationError(
                "Could not parse value[{}] to a list of integers".format(yml_rep)
            )
...
class BitfieldTableEntry(TableEntry):
    ...
    def create(name, enumeration_class, size):
        return type(
            name,
            (BitfieldTableEntry,),
            {"name": name, "enumeration_class": enumeration_class, "size": size},
        )
    ...
    @classmethod
    def from_yml_rep(cls, yml_rep):
        if isinstance(yml_rep, list) and all(
            (isinstance(x, int) or isinstance(x, str)) for x in yml_rep
        ):
            value = set()
            for entry in yml_rep:
                if isinstance(entry, str):
                    try:
                        ...
                        raise TableEntryInvalidYmlRepresentationError(
                            "Could not parse string[{}] to type[{}]".format(
            ...
            if not (0 <= yml_rep < cls.size * 0x100):
                raise TableEntryInvalidYmlRepresentationError(
                    "Integer[{}] is not valid for a bitfield of size[{}]".format(
                        yml_rep, cls.size
                    )
                )
            ...
            raise TableEntryInvalidYmlRepresentationError(
                "Expected list of bitvalues but instead got value[{}] of type[{}]".format(
                    yml_rep, type(yml_rep).__name__
                )
            )
...
class RowTableEntry(TableEntry):
    ...
    def from_schema(cls, schema, name="CustomRowTableEntry", hidden_columns=set()):
        if type(hidden_columns) == list:
            ...
            raise InvalidArgumentError(
                "Could not create RowTableEntry with invalid hidden_columns[{}]".format(
                    hidden_columns
                )
            )
    ...
    @classmethod
    def from_schema_specification(
        cls, schema_specification, name="CustomRowTableEntry", hidden_columns=set()
    ):
    ...
    def from_yml_rep(cls, yml_rep):
        ...
        for i, column in enumerate(cls.schema):
            ...
            try:
                ...
                error_message = "Column[{}] not found in yml representation".format(
                    column.name
                )
                log.debug(error_message)
                raise TableSchemaError(
                    field=column.name, cause=TableEntryMissingDataError(error_message)
                )
            ...
            try:
                ...
                log.debug(
                    "Error while parsing yml representation for column[{}]".format(
                        column.name
                    )
                )
                ...
                log.debug(
                    "Unexpected error while parsing yml representation for column[{}]".format(
                        column.name
                    )
                )
                raise TableSchemaError(field=column.name, cause=e)
...
class GenericLittleEndianRowTableEntry(RowTableEntry):
    ...
    TABLE_ENTRY_CLASS_MAP = {
        "int": (LittleEndianIntegerTableEntry, ["name", "size"]),
    ...
    def to_table_entry_class(cls, column_specification):
        ...
        if (column_specification["type"] == "int") and (
            "values" in column_specification
        ):
        ...
        elif (column_specification["type"] == "bitfield") and (
            "bitvalues" in column_specification
        ):
            ...
            try:
                ...
                raise InvalidArgumentError(
                    "Unknown table column type[{}]".format(column_specification["type"])
                )

            try:
                parameters = dict(
                    [(x, column_specification[x]) for x in parameter_list]
                )
            except KeyError:
                raise InvalidArgumentError(
                    "Column[{}] in table schema not provided with all required attributes[{}]".format(
                        column_specification["name"], parameter_list
                    )
                )
...
class Table(object):
    def __init__(self, schema, name="Anonymous Table", size=None, num_rows=None):
        ...
        if size is None and num_rows is None:
            raise InvalidArgumentError(
                "Cannot create table[{}] with null size and null num_rows".format(
                    self.name
                )
            )
    ...
    def recreate(self, num_rows=None, size=None):
        if num_rows is not None:
            ...
            if size % self.schema.size != 0:
                raise InvalidArgumentError(
                    "Cannot create table[{}] with rows of size[{}] and total size[{}]".format(
        ...
        if yml_rep is None:
            raise TableError(
                ...
                cause=TableEntryMissingDataError(
                    "No data found, please check the file"
                ),
            )
        for i in range(self.num_rows):
            try:
    ...
    def __getitem__(self, index):
        ...
        if 0 <= row < self.num_rows:
            ...
            raise IndexOutOfRangeError(
                "Cannot get row[{}] from table of size[{}]".format(row, self.num_rows)
            )
    ...
    def __setitem__(self, index, value):
        ...
        if 0 <= row < self.num_rows:
            ...
            raise IndexOutOfRangeError(
                "Cannot set row[{}] in table of size[{}]".format(row, self.num_rows)
            )


class MatrixTable(Table):
    def __init__(
        self, schema, matrix_height, name="Anonymous Table", size=None, num_rows=None
    ):
        super(MatrixTable, self).__init__(
            schema=schema, name=name, size=size, num_rows=num_rows
        )
